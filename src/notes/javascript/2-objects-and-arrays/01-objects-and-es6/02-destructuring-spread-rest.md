...
### 🕰️ The Legacy `arguments` Object (Pre-ES6)
Before the Rest operator, developers had to use the hidden `arguments` object inside normal functions to capture multiple arguments.
...
> [!WARNING]
> **Gotcha:** The `arguments` object is **NOT** available inside Arrow Functions! You *must* use the Rest operator `(...args)` to capture arguments in an arrow function.
...
**Q: What is the difference between the Spread operator and the Rest parameter?**
- **A:** They look the same (`...`), but do opposite things. Spread *expands* an iterable into individual elements (e.g., passing array items as separate arguments). Rest *collects* multiple elements into a single array (e.g., gathering function arguments).