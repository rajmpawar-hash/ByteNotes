...
These three functions are the Holy Trinity of Higher-Order Functions in JavaScript. They are special built-in array methods, meaning you can use them directly on any array you create. 

*(Note: They work because they are attached to something called `Array.prototype`. You'll learn exactly how that works in the Object-Oriented Programming section!)*
...
- **A:** No! `map`, `filter`, and `reduce` are pure functions; they return new values/arrays and leave the original array completely untouched.

**Q: When would you use `reduce()` over `map()`?**
- **A:** Use `map()` when you need to transform every element and return an array of the exact same length. Use `reduce()` when you need to condense the array into a single value (like a sum, an object, or even a different data structure).