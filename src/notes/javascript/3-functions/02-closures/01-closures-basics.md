...
- **A:** A closure is a function bundled together with its lexical environment, allowing it to remember the variables from its parent's scope even after the parent has finished executing.

**Q: Do closures capture values or references?**
- **A:** Closures capture **references** to variables, not their values at the time of creation. If the outer variable changes, the closure sees the updated value.