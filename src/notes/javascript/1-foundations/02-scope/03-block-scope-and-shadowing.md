...
### The `var` Function Exception
If you shadow a global `var` with another `var` inside a **function** (instead of a simple block), it is perfectly safe! Because `var` respects function boundaries, the inner `var` stays trapped inside the function's local execution context and leaves the global variable completely alone.

```javascript
var z = 100; // Global scope

function test() {
    var z = 10; // Function scope (shadows outer z safely)
    console.log(z); // 10
}

test();
console.log(z); // 100 (The global z is untouched!)
```
