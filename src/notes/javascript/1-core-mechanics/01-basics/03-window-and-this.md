...
**Q: Does every JS file have a `window` object?**
- **A:** Only in the browser! In Node.js, the global object is called `global`. You can use `globalThis` to access the global object universally across environments.

**Q: Do `let` and `const` attach to the global object?**
- **A:** No. Only variables declared with `var` and standard function declarations at the top level attach to the global object (`window`).