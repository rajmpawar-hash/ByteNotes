...
- **A:** By encapsulating a variable within an outer function's scope, and returning an object or inner function that interacts with that variable. The outside world cannot access the variable directly. This is known as the Module Pattern.

**Q: How do you fix the classic `var` inside a `setTimeout` loop bug without using `let`?**
- **A:** By wrapping the `setTimeout` in an IIFE (Immediately Invoked Function Expression) and passing the loop variable into it. This creates a new closure scope for every iteration, capturing the current value of the variable.