...
- **A:** When using callbacks (especially with third-party APIs), you give the API control over when, how, and if your callback executes. You lose control of your program's flow.

**Q: How do event listeners use callbacks and closures together?**
- **A:** An event listener takes a callback function. This callback often accesses variables from the scope where `addEventListener` was called, thereby forming a closure over that outer scope.