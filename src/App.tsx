...
import { Menu, X, Moon, Sun, ChevronDown, ChevronRight, ChevronLeft, Clock, Search, Copy, Check, ArrowUp } from 'lucide-react';
...
function CodeBlock({ children, className, ...rest }: any) {
  ...
  return (
    <>
      ...
      >
        {copied ? (
          <><Check size={14} className="text-green-400" /><span className="text-green-400 font-medium">Copied</span></>
        ) : (
...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  const [openFolders, setOpenFolders] = useState<Record<string, boolean>>(() => {
    ...
    if (currentItem) {
      if (currentItem.parts.length > 1) initial[currentItem.parts[0]] = true;
      if (currentItem.parts.length > 2) initial[`${currentItem.parts[0]}-${currentItem.parts[1]}`] = true;
      if (currentItem.parts.length > 3) initial[`${currentItem.parts[0]}-${currentItem.parts[1]}-${currentItem.parts[2]}`] = true;
    }
    return initial;
  });
  ...
  // Close sidebar on route change on mobile and auto-expand active folders
  useEffect(() => {
    setSidebarOpen(false);
    const container = document.getElementById('main-scroll-container');
    if (container) container.scrollTop = 0; // Instant scroll on navigation
    ...
    if (currentItem) {
      setOpenFolders(prev => {
        const next = { ...prev };
        if (currentItem.parts.length > 1) next[currentItem.parts[0]] = true;
        if (currentItem.parts.length > 2) next[`${currentItem.parts[0]}-${currentItem.parts[1]}`] = true;
        if (currentItem.parts.length > 3) next[`${currentItem.parts[0]}-${currentItem.parts[1]}-${currentItem.parts[2]}`] = true;
        return next;
      });
    }
  }, [location.pathname]);
  ...
  const filteredItems = query 
    ? navItems.filter(item => item.title.toLowerCase().includes(query) || item.route.toLowerCase().includes(query))
    : navItems;

  const filteredGroupedNav = filteredItems.reduce((acc, item) => {
    const topFolder = item.parts.length > 1 ? item.parts[0] : 'general';
    const topDisplay = topFolder.replace(/^\d+-/, '').replace(/-/g, ' ').toUpperCase();
  ...
  const currentOpenFolders = query 
    ? Object.keys(filteredGroupedNav).reduce((acc, topKey) => {
        acc[topKey] = true;
        Object.keys(filteredGroupedNav[topKey].subGroups).forEach(subKey => {
          acc[`${topKey}-${subKey}`] = true;
          if (filteredGroupedNav[topKey].subGroups[subKey].subGroups) {
            Object.keys(filteredGroupedNav[topKey].subGroups[subKey].subGroups).forEach(ssKey => {
              acc[`${topKey}-${subKey}-${ssKey}`] = true;
            });
          }
        });
        return acc;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--bg-color)] text-[var(--text-color)] selection:bg-primary-500/30">
      ...
      `}>
        ...
        <div className="px-4 mb-4 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-gray-100 dark:bg-gray-800/50 border border-[var(--border-color)] rounded-xl pl-10 pr-4 py-2 text-sm text-gray-900 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-500/50 transition-all placeholder:text-gray-400"
            />
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-4 pb-20 space-y-4">
          ...
          {Object.entries(filteredGroupedNav).map(([topKey, topGroup]) => {
            const isTopOpen = currentOpenFolders[topKey];
            return (
                ...
                {isTopOpen && (
                  <div className="ml-1 pl-3 border-l border-[var(--border-color)] space-y-3">
                    ...
                    {Object.entries(topGroup.subGroups).map(([subKey, subGroup]) => {
                      ...
                      const isSubOpen = currentOpenFolders[compoundKey];
                      return (
                          ...
                          {isSubOpen && (
                            <div className="mt-1 border-l ml-3 pl-2 border-[var(--border-color)] space-y-2">
                              ...
                              {subGroup.subGroups && Object.entries(subGroup.subGroups).map(([subSubKey, subSubGroup]) => {
                                ...
                                const isSubSubOpen = currentOpenFolders[subCompoundKey];
                                return (
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div 
          className="flex-1 overflow-y-auto p-4 lg:p-8 lg:pt-6" 
        ...
        >
          {showScrollTop && (
            <button
              onClick={scrollToTop}
              className="fixed bottom-8 right-8 p-3 bg-primary-500 text-white rounded-full shadow-lg hover:bg-primary-600 hover:shadow-primary-500/30 transition-all z-50"
