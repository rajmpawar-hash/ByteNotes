...
10. **Mandatory Code Snippets**: No walls of text. Every single concept, no matter how small or theoretical, must be immediately followed by a concise, real-world code snippet demonstrating it in action.
11. **Infrastructure & Setup Focus**: Always include an 'Infrastructure & Setup' section for new technologies. Don't just teach the syntax in a vacuum; explicitly explain how to initialize, configure, and integrate the technology into a real-world production environment (e.g., `tsconfig.json`, `package.json` scripts, initialization commands).
12. **Mermaid Compatibility (v11+)**: NEVER use double quotes inside Decision Nodes (e.g., `{"Text"}`) or Edge Labels (e.g., `-->|"Text"|`) as this breaks the Markdown renderer. Use standard rectangles `["Text"]` for decisions if quotes are needed, and unquoted edge labels `-->|Text|`.
13. **DRY Architecture (No Duplicates)**: Maintain a single source of truth for concepts. For overlapping topics (e.g., Map/Filter/Reduce in Arrays vs Functions), explain the implementation in one dedicated file and simply hyperlink (`[Link](/path/to/file)`) to it from other sections. Avoid repeating the same lesson in multiple directories.
14. **Precision in Code Examples**: Ensure that dummy data, string lengths, and hardcoded variables precisely match the console outputs in the examples (e.g., if a string is sliced, the output must mathematically match the string used). Comment clearly when mixing paradigms (e.g., explicitly stating why a parent function is regular while the inner callback is an arrow function for `this` binding).
